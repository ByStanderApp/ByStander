...
class ApiService {
  ...
  static const String _guidanceBaseUrl = 'http://localhost:5001';
  ...
  Future<GuidanceResponse> getGuidanceFromSentence(String sentence) async {
    final Uri url = Uri.parse('$_guidanceBaseUrl/generate_guidance_sentence_only');
    try {
  ...
  Future<FacilitySearchResponse> findNearbyFacilities({
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception('Failed to find facilities. Status: ${response.statusCode}, Message: $errorMessage');
      }
    } catch (e) {
